...
{
    public class TipoFaseBS : CommonBusiness
    {
        private DataAccess.TipoFase datTipoFase;
        protected string className;
        public TipoFaseBS()
        {
            className = string.Format("{0}.{1}", this.GetType().Namespace, this.GetType().Name);     
            this.datTipoFase = new DataAccess.TipoFase();
        }
        public DataTable Todos()
        {
            DataTable dt = this.datTipoFase.SelectAll();
            this.Result = this.datTipoFase.Result;
