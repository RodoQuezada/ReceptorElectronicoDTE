...
{
    public class ObjetivoBS : CommonBusiness
    {
        private DataAccess.Objetivo datObjetivo;
        protected string className;
        public ObjetivoBS()
        {
            className = string.Format("{0}.{1}", this.GetType().Namespace, this.GetType().Name);     
            this.datObjetivo = new DataAccess.Objetivo();
        }

        public DataTable SelectByJefe(int IdCabEvaluacion, int IdEvaluador)
        {
            DataTable dt = this.datObjetivo.SelectByJefe (IdCabEvaluacion, IdEvaluador);
            this.Result = this.datObjetivo.Result;
        ...
        {
            DataTable dt = this.datObjetivo.SelByEval(IdCabEvaluacion);
            this.Result = this.datObjetivo.Result;
            if (this.Result.HasError)
                return null;
        ...
        public DataTable Insert(int IdCabEvaluacion,int IdEvaluador, int IdEvaluado, int IdEstado)
        {
            ...
            try
            {
                ...
                {
                    dt = this.datObjetivo.Insert(IdCabEvaluacion, IdEvaluador, IdEvaluado, IdEstado);
                    this.Result = this.datObjetivo.Result;
                    scope.Complete();
                }
            }
